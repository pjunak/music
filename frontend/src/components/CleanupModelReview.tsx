...
export function CleanupModelReview({ trackId, catalogJobId, onResult, editionReview = false }: {
  trackId: number; catalogJobId: string; editionReview?: boolean; onResult: (result: CleanupModelResult) => void;
}) {
  ...
  return <div className="cleanup-evidence">
    ...
    }}>{editionReview ? "Ask AI about these editions" : "Review ambiguous candidates with AI"}</button>}
    ...
    {status?.available && <>
      <p>{status.model_id} can {editionReview ? "explain the supplied edition differences and suggest an edition only when supported, or abstain. You choose whether to use its advice" : "suggest one supplied candidate or abstain. Its result starts unchecked"}.</p>
      ...
      <button type="button" disabled={!consent || busy} onClick={async () => {
        setBusy(true);
        try {
          let job = await cleanupApi.reviewCandidates(trackId, catalogJobId, status.disclosure_version, editionReview);
