...
describe("TagCatalogManager", () => {
  it("offers manual editing without cleanup suggestions", async () => {
    render(<TagCatalogManager catalog={catalog} onChanged={vi.fn()} />);
    await userEvent.click(screen.getByText("Manage used tags"));
    expect(screen.getByRole("button", { name: "Rename or merge tavern" })).toBeEnabled();
    expect(screen.queryByText(/Cleanup suggestions/)).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Find suggestions" })).not.toBeInTheDocument();
  });
