...
|---|---|---|---|
| 1 | Diagnose incomplete catalog lookups using the retained safe failure details, then fix the observed request/configuration problem. Recovering missing evidence comes before relaxing matching. | Old jobs cannot recover discarded error details. Blind retries add latency and provider traffic; an unavailable response is not a no-match judgment. | Small–medium; deploy diagnostics, inspect a small development sample, then choose a bounded remedy. |
