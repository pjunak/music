...
|---|---|---|---|
| 1 | Isolate persistent Last.fm HTTP 400 responses after the GET/error-handling retest. The export is retained, but no usable API code was returned; the same Rust client receives a normal JSON error with a placeholder key locally. Assess MusicBrainz timeouts separately. | The local probe does not validate the production key or recording resource. Host/container networking and backend build differences remain possible; blind retries add latency and traffic. | Small–medium; compare the [public placeholder probe](docs/LIBRARY_METADATA_PILOT.md#when-an-http-error-has-no-api-code) from the production host and confirm its running backend image before choosing a remedy. |
