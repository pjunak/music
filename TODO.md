...
offline pilot scoring/comparison tooling, title-style preservation and release-position
review notes are implemented. This is the remaining plan for
...
|---|---|---|---|
| 1 | Finish the fixed album/soundtrack pilot with the [implemented tooling](docs/LIBRARY_METADATA_PILOT.md). All 114 development tracks now have retained baseline results. Establish recording/edition precision and validate the title-style filter on a refreshed small sample. | Independent labels require operator time; small or album-leaking samples mislead. Keep artist/release families together, report denominators and unknowns; partial evidence never counts as a complete no-match. | Small–medium; deploy the review improvements, inspect catalog positions, and complete independent recording/edition judgments while preserving the fixed holdout. |
...
| 3 | Preserve independently corroborated artist-credit spellings and recover explicitly known editions. The development runs expose Romanized-to-native-script name proposals and a confirmed reissue absent from the returned choices. Evaluate alias-assisted acceptance only after these concrete cases. | An alias search hit alone cannot equate complete credits or select an edition. Preserve artist identity, all credit members and provenance; reject version contradictions. A missing edition may reflect unavailable evidence or catalog coverage, not title normalization. | Medium–large; inspect complete credit/alias evidence and the confirmed edition before relaxing acceptance or adding normalization. |
