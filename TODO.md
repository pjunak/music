...
|---|---|---|---|
| 1 | Refresh the fixed album/soundtrack development pilot with the [implemented tooling](docs/LIBRARY_METADATA_PILOT.md). All 114 tracks have retained baseline results; check edition recovery, composer credits, source review and availability after deployment. Establish recording/edition precision with independent judgments. | Small or album-leaking samples mislead; source imports must not feed expected answers into recognition scoring. Keep families together and report unknowns and partial evidence separately. | Small–medium; deployment and refreshed exports are the next boundary. Preserve the fixed holdout. If MusicBrainz failures persist, compare deployment/network diagnostics before changing matching or retry bounds. |
...
| 6 | One additional catalog or paid recognition fallback. May resolve cases missed by current sources. | Coverage is unknown; credentials, requests, attribution, persistence/export terms and possible charges add maintenance. Compare incremental useful matches on the same unresolved cohort. | Medium–large; consult on a concrete provider, permitted data, and request/cost cap. |
