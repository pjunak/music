...
retrieval, corroborated sibling discovery and offline pilot tooling are implemented. This is the remaining plan for
...
| 2 | Artist aliases/transliterations, explicit version evidence and discovery across disc folders. Helps tracks whose own text, album hints and same-folder siblings remain insufficient. | More requests and potential false matches; transliteration can collapse distinct names and disc grouping can mix editions. Bound expansion, retain query provenance and reject version/identity contradictions. Conservative encoding repair is an optional substep, never blanket normalization. | Large; evaluate against the fixed benchmark before relaxing acceptance. |
