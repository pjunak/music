...
retrieval, corroborated sibling discovery, catalog artist aliases and offline pilot tooling are implemented. This is the remaining plan for
...
| 2 | Evaluate alias-assisted identity acceptance, explicit version evidence and discovery across disc folders. Helps tracks still unresolved after catalog alias retrieval, album hints and same-folder siblings. | Accepting alternate artist spellings can collapse distinct identities; disc grouping can mix editions. Preserve full credits, retain provenance and reject version/identity contradictions. Conservative encoding repair is an optional substep, never blanket normalization. | Large; use independently labeled misses before relaxing acceptance or adding speculative normalization. |
