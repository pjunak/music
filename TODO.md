...
|---|---|---|---|
| 1 | Retest the corrected Last.fm read request and HTTP/API-code handling on the same small development sample, then resolve any reported source problem. Assess MusicBrainz timeouts separately. Recovering missing evidence comes before relaxing matching. | HTTP 400 alone does not identify a key, parameter or resource problem. Blind retries add latency and provider traffic; an unavailable response is not a no-match judgment. | Small–medium; deploy the request/error fix, retain the new export, then choose a bounded remedy from the numeric code. |
