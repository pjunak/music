...
- Keep the two existing OpenAI-compatible adapter IDs and their behavior stable. They remain the
  broad compatibility path for OpenAI-shaped services.
- Add explicit Google Gemini profiles. They pin the documented Google AI Studio base URL,
  ...
  `reasoning_effort=high/none`; Provider default sends no override. Standard and strict-schema
  profiles remain separate so conformance proves the exact selected mode.
...
- Continue reducing upstream failures to bounded machine-readable codes. Provider validation
  failures use `invalid_request`; raw provider messages, prompts, responses, and credentials remain
  private.
